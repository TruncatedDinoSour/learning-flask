    ...
    if request.method == 'POST':
        ...
        if user:
            if check_password_hash(user.password, password):
                ...
                flash(f'Login failed - invalid credentials', category='error')
        else:
...
def signup():
    if request.method == 'POST':
        ...
        if user:
            flash(f'Seems like the user already exists', category='error')
        elif len(email) < 6:
            ...
            try:
                ...
                flash(f'[ CRITICAL ] account creation failed due to [ {e} ], contact the site owner', category='error')
